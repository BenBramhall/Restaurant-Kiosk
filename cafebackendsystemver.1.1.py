...
while True:
    ...
    if choice == "Remove":
        ...
        if item_to_remove in orders and tax:
            orders.remove(item_to_remove) and tax.remove(item_to_remove)
            # Subtract the price
            total_to_add -= menu.get(item_to_remove, 0)
            total_to_add -= tax.get(item_to_remove, 0)
    ...
    # Add item
    if choice in menu and tax:
        price = menu[choice] + tax [choice] 
        total_to_add += price 
...
print("Thank you for your time and business.")